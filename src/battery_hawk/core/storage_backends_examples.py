...
# Constants
MAX_READINGS_LIMIT = 10000  # Maximum number of readings to keep in JSON file
...
class JSONFileStorageBackend(BaseStorageBackend):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self.logger.exception("Failed to connect to JSON file storage")
            return False
        else:
            return True
    ...
    async def store_reading(
        ...
        try:
            ...
            # Keep only last MAX_READINGS_LIMIT readings to prevent file from growing too large
            if len(readings) > MAX_READINGS_LIMIT:
                readings = readings[-MAX_READINGS_LIMIT:]
    ...
    async def get_recent_readings(
        ...
        try:
            ...
            self.logger.debug(
                "Retrieved %d readings for device %s from JSON file",
                len(result),
                device_id,
            )
        except Exception:
    ...
    async def get_vehicle_summary(
        ...
        try:
            ...
            temperatures = [
                r.get("temperature", 0) for r in vehicle_readings if "temperature" in r
            ]
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            self.logger.exception("JSON file storage health check failed")
            self.connected = False
            return False
        else:
            return True
    ...
    def _load_readings(self) -> list[dict[str, Any]]:
        ...
        try:
            if self.readings_file and self.readings_file.exists():
                with open(self.readings_file) as f:
                    return json.load(f)
...
class NullStorageBackend(BaseStorageBackend):
    ...
    @property
    def capabilities(self) -> set[str]:
        """Return supported capabilities for null backend."""
        return set()

    async def connect(self) -> bool:
        """Connect to the null storage backend (always succeeds)."""
        self.connected = True
        self.logger.info("Connected to null storage (data will be discarded)")
        return True

    async def disconnect(self) -> None:
        """Disconnect from the null storage backend (no-op)."""
        self.connected = False
    ...
    async def store_reading(
        self,
        device_id: str,
        vehicle_id: str,  # noqa: ARG002
        device_type: str,  # noqa: ARG002
        reading: dict[str, Any],  # noqa: ARG002
    ) -> bool:
        """Store a battery reading in the null storage backend (discard data)."""
        self.metrics.total_writes += 1
        self.metrics.successful_writes += 1
    ...
    async def get_recent_readings(
        ...
        """Get recent readings for a device from null storage backend."""
        self.metrics.total_reads += 1
        self.metrics.successful_reads += 1
        return []
    ...
    async def get_vehicle_summary(
        ...
        """Get summary statistics for a vehicle from null storage backend."""
        return {
            "vehicle_id": vehicle_id,
            "period_hours": hours,
            "avg_voltage": 0.0,
    ...
    async def health_check(self) -> bool:
        """Perform a health check on null storage backend (always succeeds)."""
        return True
