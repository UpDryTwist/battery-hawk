...
class MQTTInterface:
    ...
    def __init__(self, config_manager: ConfigManager) -> None:
        ...
        self._message_queue: deque[QueuedMessage] = deque(maxlen=self._reconnection_config.message_queue_size)
    ...
    def _get_mqtt_config(self) -> dict[str, Any]:
        ...
        missing_fields = [field for field in required_fields if not mqtt_config.get(field)]

        if missing_fields:
            raise ValueError(f"Missing required MQTT configuration fields: {missing_fields}")

        # Validate port range
        port = mqtt_config.get("port", 1883)
        if not isinstance(port, int) or port < 1 or port > MAX_PORT_NUMBER:
            raise ValueError(f"Invalid MQTT port: {port}. Must be between 1 and {MAX_PORT_NUMBER}")
    ...
    def _on_config_change(self, section: str, _config: dict[str, Any]) -> None:
        ...
        if section == "system":
            ...
            try:
                ...
                mqtt_fields = ["broker", "port", "username", "password", "tls", "topic_prefix"]
                config_changed = any(
                ...
                reconnection_changed = (
                    old_reconnection_config.message_queue_size != self._reconnection_config.message_queue_size
                )

                if reconnection_changed:
                    # Update message queue size if it changed
                    new_queue = deque(self._message_queue, maxlen=self._reconnection_config.message_queue_size)
                    self._message_queue = new_queue

                if config_changed and self._connection_state == ConnectionState.CONNECTED:
                    self.logger.info("MQTT configuration changed, reconnecting...")
                    asyncio.create_task(self._initiate_reconnection())
    ...
    async def _connect_with_retry(self) -> None:
        ...
        for attempt in range(max_retries + 1):
            ...
            try:
                ...
                self.logger.info(
                    "Successfully connected to MQTT broker at %s:%d (attempt %d/%d)",
                    broker,
                    port,
                    attempt + 1,
                    max_retries + 1,
            ...
            try:
                ...
                if self._connection_state == ConnectionState.CONNECTED:
                    ...
                    if not self._client or not hasattr(self._client, '_client'):
                        self.logger.warning("MQTT client became invalid, initiating reconnection")
                ...
                self.logger.error("Error in health check: %s", e)
        ...
        while not self._shutdown_event.is_set() and attempt < max_retries:
            try:
                ...
                if attempt < max_retries:
                    ...
                    self.logger.error(
                        "Reconnection failed after %d attempts: %s",
                        max_retries,
                        e,
                    )
                    self._connection_state = ConnectionState.FAILED
                ...
                self.logger.error("Unexpected error during reconnection: %s", e)
                attempt += 1
                if attempt >= max_retries:
                    self._connection_state = ConnectionState.FAILED
                    return
    ...
    async def _publish_message(self, msg: QueuedMessage) -> None:
        ...
        try:
            ...
            if self._is_connection_error(e):
                self.logger.warning("Connection error detected, initiating reconnection...")
                asyncio.create_task(self._initiate_reconnection())
    ...
    async def _process_message_queue(self) -> None:
        ...
        async with self._queue_lock:
            ...
            while self._message_queue and self._connection_state == ConnectionState.CONNECTED:
                ...
                try:
                    ...
                    self.logger.error(
                        "Dropping message to topic '%s' due to serialization error: %s",
                        msg.topic,
                        e,
                    )
                    failed_count += 1
    ...
    async def _handle_messages(self) -> None:
        ...
        try:
            ...
            if self._is_connection_error(e):
                self.logger.warning("Connection error in message handling, initiating reconnection...")
                asyncio.create_task(self._initiate_reconnection())

    def _is_connection_error(self, error: Exception) -> bool:
        """Check if error is a connection-related error."""
        return isinstance(error, (MqttError, OSError, ConnectionError, asyncio.TimeoutError))
...
class MQTTPublisher:
    ...
    async def publish_device_reading(
        ...
        *,
        ...
        try:
            ...
            self.logger.error(
                "Failed to publish device reading for %s: %s",
                device_id,
                e,
            )
            raise
    ...
    async def publish_device_status(
        ...
        *,
        ...
        try:
            ...
            self.logger.error(
                "Failed to publish device status for %s: %s",
                device_id,
                e,
            )
            raise
    ...
    async def publish_vehicle_summary(
        ...
        try:
            ...
            self.logger.error(
                "Failed to publish vehicle summary for %s: %s",
                vehicle_id,
                e,
            )
            raise
    ...
    async def publish_system_status(
        ...
        try:
            # Use QoS 2 for system status (critical)
            # Retain system status so new subscribers get last known state
            # Override QoS for this critical message type
            original_qos = self.mqtt_interface._mqtt_config.get("qos", 1)
            self.mqtt_interface._mqtt_config["qos"] = 2
            ...
            try:
                ...
                self.mqtt_interface._mqtt_config["qos"] = original_qos
...
class MQTTEventHandler:
    ...
    def _register_core_engine_handlers(self) -> None:
        ...
        device_discovered_handler = self._create_async_handler(self.on_device_discovered)
        self.core_engine.add_event_handler("device_discovered", device_discovered_handler)
        self._registered_handlers["core_device_discovered"] = device_discovered_handler

        # Vehicle associated handler
        vehicle_associated_handler = self._create_async_handler(self.on_vehicle_associated)
        self.core_engine.add_event_handler("vehicle_associated", vehicle_associated_handler)
        self._registered_handlers["core_vehicle_associated"] = vehicle_associated_handler
    ...
    def _register_state_manager_handlers(self) -> None:
        ...
        connection_handler = self._create_state_handler(self.on_device_connection_change)
        self.core_engine.state_manager.subscribe_to_changes("connection", connection_handler)
    ...
    def _create_state_handler(self, handler_method: Callable) -> Callable:
        ...
        def state_wrapper(
            ...
            try:
                ...
                self.logger.error(
                    "Error in state event handler %s: %s",
                    handler_method.__name__,
                    e,
                )
    ...
    async def on_device_discovered(self, event_data: dict[str, Any]) -> None:
        ...
        try:
            ...
            await self.mqtt_publisher.mqtt_interface.publish(
                topic, discovery_payload, retain=False
            )
            ...
            self.logger.error(
                "Failed to handle device discovered event: %s", e
            )
    ...
    async def on_device_reading(
        ...
        try:
            ...
            vehicle_id = new_state.vehicle_id or device_info.get("vehicle_id") if device_info else None
            ...
            self.logger.error(
                "Failed to handle device reading event: %s", e
            )
    ...
    async def on_device_status_change(
        ...
        try:
            ...
            self.logger.error(
                "Failed to handle device status change event: %s", e
            )
    ...
    async def on_device_connection_change(
        ...
        try:
            ...
            self.logger.error(
                "Failed to handle device connection change event: %s", e
            )
    ...
    async def on_vehicle_associated(self, event_data: dict[str, Any]) -> None:
        ...
        try:
            ...
            await self.mqtt_publisher.mqtt_interface.publish(
                topic, association_payload, retain=False
            )
            ...
            self.logger.error(
                "Failed to handle vehicle association event: %s", e
            )
    ...
    async def on_vehicle_update(
        ...
        try:
            ...
            self.logger.error(
                "Failed to handle vehicle update event: %s", e
            )
    ...
    async def on_system_shutdown(self, event_data: dict[str, Any]) -> None:
        ...
        try:
            ...
            await self.mqtt_publisher.mqtt_interface.publish(
                "system/shutdown", shutdown_payload, retain=True
            )
            ...
            self.logger.error(
                "Failed to handle system shutdown event: %s", e
            )
    ...
    async def on_system_status_change(self, status_data: dict[str, Any]) -> None:
        ...
        try:
            ...
            self.logger.error(
                "Failed to handle system status change: %s", e
            )
    ...
    async def _update_vehicle_summary(self, vehicle_id: str) -> None:
        ...
        try:
            ...
            # Get all devices associated with this vehicle
            vehicle_devices = []
            connected_count = 0
            total_voltage = 0.0
            total_capacity = 0.0
            ...
            all_states = self.core_engine.state_manager.get_all_device_states()
            ...
            # Cache the summary to avoid redundant updates
            # Create a cache-friendly version without time-based fields for comparison
            def remove_time_fields(data: dict[str, Any]) -> dict[str, Any]:
            ...
            cache_data = remove_time_fields(summary_data)
            cache_key = vehicle_id
            cached_data = remove_time_fields(self._vehicle_summary_cache.get(cache_key, {}))

            if cache_key not in self._vehicle_summary_cache or cached_data != cache_data:
                self._vehicle_summary_cache[cache_key] = summary_data.copy()

                # Publish vehicle summary
                await self.mqtt_publisher.publish_vehicle_summary(vehicle_id, summary_data)
            ...
            self.logger.error(
                "Failed to update vehicle summary for %s: %s",
                vehicle_id,
                e,
            )
