...
from battery_hawk.core.storage import InfluxDBStorageBackend, ConnectionState
...
async def demo_error_handling():
    ...
    try:
        ...
        test_readings = [
            {"device_id": "AA:BB:CC:DD:EE:01", "voltage": 12.6, "current": 2.5, "temperature": 22.0},
            {"device_id": "AA:BB:CC:DD:EE:02", "voltage": 12.4, "current": 1.8, "temperature": 24.0},
            {"device_id": "AA:BB:CC:DD:EE:03", "voltage": 12.5, "current": 2.3, "temperature": 23.0},
            {"device_id": "AA:BB:CC:DD:EE:04", "voltage": 12.7, "current": 1.9, "temperature": 21.0},
            {"device_id": "AA:BB:CC:DD:EE:05", "voltage": 12.3, "current": 2.1, "temperature": 25.0},
        ]
        ...
        for i, reading_data in enumerate(test_readings):
            ...
            logger.info(f"Reading {i+1}: {status} - Device: {reading_data['device_id']}")
        ...
        if buffer_size > 0:
            ...
            if storage.connected:
                ...
                logger.info(f"Flush completed: {flushed} flushed, {remaining_buffer} remaining")
            else:
        ...
        logger.info(f"Background tasks after shutdown: {len(storage._background_tasks)}")
        logger.info(f"Shutdown event set: {storage._shutdown_event.is_set()}")
