...
async def main() -> None:
    ...
    try:
        ...
        messages_to_send = [
            {"topic": "test/device1", "data": {"voltage": 12.6, "timestamp": datetime.now().isoformat()}},
            {"topic": "test/device2", "data": {"voltage": 12.4, "timestamp": datetime.now().isoformat()}},
            {"topic": "test/system", "data": {"status": "running", "timestamp": datetime.now().isoformat()}},
        ]
        ...
        logger.info("Queue status: %d messages queued, %d published", 
                   stats["messages_queued"], stats["messages_published"])

        # 3. Demonstrate connection state monitoring
        logger.info("\n=== Connection State Monitoring ===")
        logger.info("Current connection state: %s", mqtt_interface.connection_state.value)
        ...
        if mqtt_interface.connection_state != ConnectionState.CONNECTED:
            ...
            logger.info("If MQTT broker becomes available, messages will be automatically published")
...
async def demonstrate_reconnection_scenario() -> None:
    ...
    try:
        ...
        for i in range(5):
            await mqtt_interface.publish(f"demo/message/{i}", {
                "message_id": i,
        ...
        try:
            ...
            logger.info("Total messages published: %d", final_stats["messages_published"])
