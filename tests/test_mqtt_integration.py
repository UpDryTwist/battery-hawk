...
from battery_hawk.mqtt import MQTTInterface, MQTTPublisher, MQTTEventHandler, MQTTTopics
...
class MockConfigManager:
    ...
    def add_listener(self, handler) -> None:
        """Add configuration change listener (for MQTT client compatibility)."""
        self._change_handlers.append(handler)
...
class TestMQTTIntegration:
    ...
    def test_topic_structure_compliance(self, mqtt_interface: MQTTInterface) -> None:
        ...
        assert topics.device_reading(mac) == "test_batteryhawk/device/AA:BB:CC:DD:EE:FF/reading"
        assert topics.device_status(mac) == "test_batteryhawk/device/AA:BB:CC:DD:EE:FF/status"
        
        # Test vehicle topics
        vehicle_id = "my_vehicle"
        assert topics.vehicle_summary(vehicle_id) == "test_batteryhawk/vehicle/my_vehicle/summary"
    ...
    async def test_complete_device_workflow(
        ...
        with patch.object(mqtt_interface, '_client') as mock_client:
            ...
            mqtt_interface._connection_state = mqtt_interface._connection_state.CONNECTED
    ...
    async def test_event_handler_integration(
        ...
        with patch.object(mqtt_event_handler.mqtt_publisher.mqtt_interface, '_client') as mock_client:
            mock_client.publish = AsyncMock()
            mqtt_event_handler.mqtt_publisher.mqtt_interface._connection_state = \
                mqtt_event_handler.mqtt_publisher.mqtt_interface._connection_state.CONNECTED
    ...
    def test_custom_topic_prefix(self) -> None:
        ...
        mac = "AA:BB:CC:DD:EE:FF"
        assert custom_topics.device_reading(mac) == "custom_prefix/device/AA:BB:CC:DD:EE:FF/reading"
