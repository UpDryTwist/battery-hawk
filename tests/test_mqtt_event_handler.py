...
class TestMQTTEventHandler:
    ...
    @pytest.fixture
    def mock_core_engine(self) -> BatteryHawkCore:
        ...
        mock_state_manager.get_all_device_states = MagicMock(return_value={})
        ...
        mock_vehicle_registry.get_vehicle = MagicMock(return_value={"name": "Test Vehicle"})
        mock_engine.vehicle_registry = mock_vehicle_registry
        
        return mock_engine

    @pytest.fixture
    def mock_mqtt_publisher(self) -> MQTTPublisher:
    ...
    def test_register_all_handlers(
        ...
        mock_core_engine.add_event_handler.assert_any_call(
            "device_discovered", event_handler._registered_handlers["core_device_discovered"]
        )
        mock_core_engine.add_event_handler.assert_any_call(
            "vehicle_associated", event_handler._registered_handlers["core_vehicle_associated"]
        )
        mock_core_engine.add_event_handler.assert_any_call(
            "system_shutdown", event_handler._registered_handlers["core_system_shutdown"]
        )
        ...
        mock_core_engine.state_manager.subscribe_to_changes.assert_any_call(
            "reading", event_handler._registered_handlers["state_reading"]
        )
        mock_core_engine.state_manager.subscribe_to_changes.assert_any_call(
            "status", event_handler._registered_handlers["state_status"]
        )
        mock_core_engine.state_manager.subscribe_to_changes.assert_any_call(
            "connection", event_handler._registered_handlers["state_connection"]
        )
        mock_core_engine.state_manager.subscribe_to_changes.assert_any_call(
            "vehicle", event_handler._registered_handlers["state_vehicle"]
        )
    ...
    async def test_on_device_connection_change(
        ...
        await event_handler.on_device_connection_change(mac_address, new_state, old_state)
    ...
    async def test_update_vehicle_summary(
        ...
        mock_core_engine.state_manager.get_all_device_states.return_value = {
            "AA:BB:CC:DD:EE:FF": device_state
        }
