...
def device_factory():
    ...
    factory = MagicMock(spec=DeviceFactory)
    return factory


@pytest.fixture
def auto_config_service(config_manager, device_factory):
...
class TestAutoConfigurationService:
    ...
    def test_is_enabled_true(self, auto_config_service):
        """Test that auto-configuration is enabled when configured."""
        assert auto_config_service.is_enabled() is True

    def test_is_enabled_false(self, config_manager, device_factory):
    ...
    def test_is_enabled_default_true(self, config_manager, device_factory):
    ...
    def test_is_enabled_missing_config(self, config_manager, device_factory):
        """Test that auto-configuration defaults to enabled when config is missing."""
        config_manager.get_config.return_value = {}
        service = AutoConfigurationService(config_manager, device_factory)
        assert service.is_enabled() is True

    def test_get_confidence_threshold(self, auto_config_service):
    ...
    def test_should_auto_configure_device_no_type(
        ...
        result = auto_config_service.should_auto_configure_device(
            "50:54:7B:81:33:39", sample_device_info, None
        )
        assert result is False
    ...
    def test_generate_device_name_with_template(
        ...
        name = auto_config_service.generate_device_name(
            "50:54:7B:81:33:39", "BM6", sample_device_info
        )
    ...
    def test_get_polling_interval_device_specific(self, auto_config_service):
        """Test getting device-specific polling interval."""
        interval = auto_config_service.get_polling_interval("BM6")
        assert interval == 1800

    def test_get_polling_interval_default(self, auto_config_service):
    ...
    async def test_auto_configure_device_success(
        ...
        result = await auto_config_service.auto_configure_device(
            "50:54:7B:81:33:39", sample_device_info, device_registry
        )
    ...
    async def test_process_discovered_devices(
        ...
        results = await auto_config_service.process_discovered_devices(
            discovered_devices, device_registry
        )
...
class TestDefaultDeviceTypeRule:
    ...
    def test_evaluate_with_detected_type(self, config_manager, sample_device_info):
    ...
    def test_evaluate_without_detected_type(self, config_manager, sample_device_info):
    ...
    def test_evaluate_disabled_device_type(self, config_manager, sample_device_info):
        """Test rule evaluation for disabled device type."""
        config_manager.get_config.return_value = {
            "discovery": {
                "auto_configure": {
                    "rules": {"BM6": {"auto_configure": False}}
                }
            }
        }
        rule = DefaultDeviceTypeRule(config_manager)
...
class TestAutoConfigurationRulesEngine:
    ...
    def test_initialization(self, config_manager):
        ...
        assert engine.rules[0].priority >= engine.rules[1].priority  # Sorted by priority

    def test_evaluate_device(self, config_manager, sample_device_info):
    ...
    def test_evaluate_device_no_rules_match(self, config_manager, sample_device_info):
...
class TestAutoConfigurationEnvironmentVariables:
    ...
    def test_environment_variable_disable(self, device_factory):
    ...
    def test_environment_variable_enable(self, device_factory):
    ...
    def test_environment_variable_confidence_threshold(self, device_factory):
        """Test that confidence threshold can be set via environment variable."""
        # Set environment variable for confidence threshold
        os.environ["BATTERYHAWK_SYSTEM_DISCOVERY_AUTO_CONFIGURE_CONFIDENCE_THRESHOLD"] = "0.9"
        ...
        try:
            ...
            # Clean up environment variable
            os.environ.pop("BATTERYHAWK_SYSTEM_DISCOVERY_AUTO_CONFIGURE_CONFIDENCE_THRESHOLD", None)
